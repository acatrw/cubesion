...
public sealed class BlobFloorPlanBuilderSystem : BaseWorldSystem
{
    ...
    {
        PlaceFloorplanTiles(component, (uid, Comp<MapGridComponent>(uid)));
    }

    private void PlaceFloorplanTiles(BlobFloorPlanBuilderComponent comp, Entity<MapGridComponent> gridEnt)
    {
        ...
        _maps.SetTiles(gridEnt.Owner, gridEnt.Comp, taken.Select(x => (x.Key, x.Value)).ToList());
    }
}
