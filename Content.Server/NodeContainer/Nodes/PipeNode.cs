    ...
    public partial class PipeNode : Node, IGasMixtureHolder, IRotatableNode
    {
            ...
            IEntityManager entMan)
        {
            ...
            for (var i = 0; i < PipeDirectionHelpers.PipeDirections; i++)
            {
                ...
                foreach (var pipe in LinkableNodesInDirection(pos, pipeDir, (xform.GridUid!.Value, grid), entMan.System<SharedMapSystem>(), nodeQuery))
                {
                    yield return pipe;
                }
            }
        ...
        private IEnumerable<PipeNode> LinkableNodesInDirection(Vector2i pos, PipeDirection pipeDir, Entity<MapGridComponent> grid,
            SharedMapSystem maps, EntityQuery<NodeContainerComponent> nodeQuery)
        {
            foreach (var pipe in PipesInDirection(pos, pipeDir, grid, maps, nodeQuery))
            {
            ...
            SharedMapSystem maps, EntityQuery<NodeContainerComponent> nodeQuery)
        {
            ...
            foreach (var entity in maps.GetAnchoredEntities(grid.Owner, grid.Comp, offsetPos))
            {
