...
public sealed partial class BiomeSystem
{
    ...
    private CompletionResult AddLayerCallbackHelp(IConsoleShell shell, string[] args)
    {
        ...
        if (args.Length == 3)
        {
            if (int.TryParse(args[0], out var mapInt))
            {
                ...
                if (TryComp<BiomeComponent>(_mapSystem.GetMap(mapId), out var biome))
                {
    ...
    private void AddMarkerLayerCallback(IConsoleShell shell, string argstr, string[] args)
    {
        ...
        if (!_mapSystem.MapExists(mapId) || !TryComp<BiomeComponent>(_mapSystem.GetMap(mapId), out var biome))
        {
            return;
        }
