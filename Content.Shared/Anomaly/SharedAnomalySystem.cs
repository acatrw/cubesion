...
public abstract class SharedAnomalySystem : EntitySystem
{
    ...
    public List<TileRef>? GetSpawningPoints(EntityUid uid, float stability, float severity, AnomalySpawnSettings settings, float powerModifier = 1f, float minAmountOffset = 0, float maxAmountOffset = 0)
    {
        ...
        var tilerefs = _maps.GetLocalTilesIntersecting(xform.GridUid.Value, grid,
        ...
        while (resultList.Count < amount)
        {
            ...
            {
                var valid = true;
                foreach (var ent in _maps.GetAnchoredEntities(xform.GridUid.Value, grid, tileref.GridIndices))
                {
