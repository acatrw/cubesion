...
internal struct CustomRichTextEntry
{
        ...
        float lineHeightScale = 1)
    {
        ...
        var boxTop = baseLineBase.Y - boxPadding;
        var boxBottom = Math.Max(boxTop,
            baseLine.Y - GetLineHeight(defaultFont, uiScale, lineHeightScale) + boxPadding);

        return new UIBox2(
                new Vector2(drawBox.Left + (margin - boxPadding) - sPixelWidth, boxTop),
                new Vector2(drawBox.Right - (margin - boxPadding) - sPixelWidth, boxBottom));
    }
