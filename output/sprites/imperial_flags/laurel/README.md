...
on Taleryn's reference. Purple cloth, gold trim and the five distinct
banner silhouettes are retained.

Sprited by Taleryn for Hullrot:Eclipsion. Variant names are listed in
`variants.json`. Each `*-source.png` is the final full-resolution RGBA artwork. `preview.png` shows final sprites at 6x and native
